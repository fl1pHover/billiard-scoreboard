...
export default function PlayerList() {
  ...
  const toggleSelect = (name: string) => {
    setSelectedPlayers((prev) => (prev.includes(name) ? prev.filter((n) => n !== name) : prev.length < 2 ? [...prev, name] : prev));
  };
          ...
          <ScrollArea className="h-[50vh] w-full rounded-md border gap-y-0.5">
            {state.players.length === 0 ? (
              ...
              state.players.map((p, i) => {
                ...
                return (
                  <div key={i} onClick={() => toggleSelect(p.playerName)} className={`cursor-pointer border p-2 transition ${selected ? "bg-primary text-white" : "hover:bg-muted hover:text-foreground"}`}>
                    {p.playerName} — {p.experience}y {p.isVeteran ? "(Veteran)" : "(Newbie)"}
                  </div>
                );
              })
            )}
            ...
            {selectedPlayers.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {selectedPlayers.map((name, index) => {
                  const player = state.players.find((p) => p.playerName === name);
                  if (!player) return null;
                  return (
                    <div key={index} className="p-2 border rounded-lg bg-background text-center shadow">
                      <p className="font-medium">{player.playerName}</p>
                      <p className="text-sm text-gray-500">{player.experience} жил</p>
                      <p className="text-sm">{player.isVeteran ? "Veteran" : "Newbie"}</p>
                    </div>
                  );
                })}
              </div>
        ...
        <section className="p-4 border rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            {activeMatch.map((name, index) => (
              <div key={index} className="p-4 border rounded-lg bg-background text-center shadow">
